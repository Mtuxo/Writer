function validateMaster(master) {
  ...
  for (const section of sections) if (!Array.isArray(master.bible[section])) master.bible[section] = [];
...
async function callBridge(action, master) {
  ...
  if (!/^https:\/\/script\.google\.com\//i.test(url)) throw new Error('GOOGLE_BIBLE_BRIDGE_URL must be the deployed Apps Script /exec URL.');
  const payload = { action, resource: 'bible', secret };
  ...
  let data; try { data = JSON.parse(text); } catch { throw new Error('Google Drive bridge returned an unreadable response. Redeploy the Apps Script web app.'); }
...
export default async function handler(req,res) {
  ...
  } catch (err) { return res.status(500).json({error:err?.message || 'Google Drive Bible sync failed.'}); }
}
